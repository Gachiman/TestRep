...
int main(int argc, char* argv[]) {
    ...
    MPI_Type_vector(height, width / ProcNum + 2, width, MPI_INT, &mpi_LANE1);
    MPI_Type_commit(&mpi_LANE1);

    MPI_Datatype mpi_LANE2;
    MPI_Type_vector(height, width / ProcNum + 3, width, MPI_INT, &mpi_LANE2);
    ...
    MPI_Type_free(&mpi_LANE1);
    MPI_Type_free(&mpi_LANE2);
    ...
    for (int i = 0; i < height; i++)
        for (int j = 1; j < size1 + 2; j++) {
            ...
            result[i * width + j] = Clamp(round(tmp), 0, 255);
        }
